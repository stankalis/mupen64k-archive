 ...
 //            MessageBox( hwnd, "", "", MB_OK );
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                ...
                case IDC_WEBSITE:
                     ShellExecute(NULL, "open", "http://okaycreations.com", NULL, NULL, SW_SHOWNORMAL);
                break;
            }
        break;
        default:
    ...
    switch(Message)
    {
        ...
        case WM_INITDIALOG:
            ...
            while(get_plugin_type() != -1) {
                ...
                case PLUGIN_TYPE_CHEAT:
                    SendDlgItemMessage(hwnd, IDC_COMBO_CHEAT, CB_ADDSTRING, 0, (LPARAM)next_plugin());
                    break;                                
                default:
                    next_plugin();
                }
            ...
            // Set cheat plugin
            index = SendDlgItemMessage(hwnd, IDC_COMBO_CHEAT, CB_FINDSTRINGEXACT, 0, (LPARAM)cheat_name);
            if (index!=CB_ERR) {
            SendDlgItemMessage(hwnd, IDC_COMBO_CHEAT, CB_SETCURSEL, index, 0);
            }
            else    {
                  SendDlgItemMessage(hwnd, IDC_COMBO_CHEAT, CB_SETCURSEL, 0, 0);
                  SendDlgItemMessage(hwnd, IDC_COMBO_CHEAT, CB_GETLBTEXT, 0, (LPARAM)cheat_name);
                }
            ...
            if(emu_launched) {
                ...
                EnableWindow( GetDlgItem(hwnd,IDC_COMBO_CHEAT), FALSE );  
            }
            ...
            switch(LOWORD(wParam))
            {
                ...
                case IDSOUNDABOUT:
                 ...
                 case IDCHEATCONFIG:
                     ...
                     ReadComboBoxValue( hwnd, IDC_COMBO_CHEAT, path_buffer);
                     exec_config(path_buffer);
                     break;
                case IDCHEATTEST:
                     hwnd_plug = hwnd;
        ...
        case WM_NOTIFY:
		             if (((NMHDR FAR *) lParam)->code == PSN_APPLY) {
                         ...
                         ReadComboBoxValue( hwnd, IDC_COMBO_CHEAT, cheat_name);
                         WriteCfgString("Plugins","Cheat",cheat_name);
                     }
                     break;
        default:
            return FALSE;
