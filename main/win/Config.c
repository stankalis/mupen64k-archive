...
void LoadConfig()
{
   ...
   // Setup
    if (ReadCfgInt("Setup","Run",0) == 1)
    {
           char TempStr[MAX_PATH];                         
           sprintf(TempStr,"%ssetup.txt",AppPath);
           ShellExecute(mainHWND, "open", TempStr, NULL, NULL, SW_SHOWNORMAL);
           WriteCfgInt("Setup","Run",0);
    }
   ...
   //Window position and size 
    ...
    Config.isMaximized = ReadCfgInt("Window","Maximized",0); 
    ...
    // Netplay
    ReadCfgString("NetPlay","SupraClient",Config.SupraClient, Config.SupraClient);
    ReadCfgString("NetPlay","KailleraClient",Config.KailleraClient,Config.KailleraClient);    
...
void saveWindowSettings()
{
	...
	Config.isMaximized = ((isMaximized & WS_MAXIMIZE) ? 1 : 0);
    ...
    WriteCfgInt("Window","Maximized",Config.isMaximized);
}
