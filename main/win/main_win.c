...
//extern DWORD WINAPI LaunchKaillera(LPVOID lpParam);

// extern static DWORD WINAPI KailleraThread;

// Kaillera Setup (REMEMBER TO ORGANIZE)
  extern char *TargetKailleraClient;
// End
...
static CHEAT_INFO dummy_cheat_info;
static CHEAT_INFO cheat_info;
...
BOOL (*initiateCheat)(CHEAT_INFO Cheat_Info);
void (*romClosed_cheat)();
void (*romOpen_cheat)();
void (*closeDLL_cheat)();
...
static BOOL dummy_initiateCheat(CHEAT_INFO Cheat_Info) { return TRUE; }
...
void SetupDummyGraphics()
{
     ...
     dummy_gfx_info.hStatusBar = hStatus; //(Config.GuiStatusbar == 1 ? NULL : NULL);//hStatus ;
...
void SetupDummyInfo()
{
	...
	dummy_rsp_info.ShowCFB = showCFB;
     ...
     /////// CHEAT /////////////////////////
     dummy_cheat_info.hwnd = mainHWND;
     dummy_cheat_info.hinst = app_hInstance;
   //  dummy_cheat_info.MemoryBswaped = TRUE;
     dummy_cheat_info.HEADER = (BYTE*)DummyHeader ;
     dummy_cheat_info.RDRAM = (BYTE*)rdram;
...
{
    static char gfx_temp[100],input_temp[100],sound_temp[100],rsp_temp[100],cheat_temp[100];
...
int get_plugin_top(plugins *p, char *name)
{
   if (!p->next) return 0;
   ...
   if (!strcmp(p->next->plugin_name, name))
         ...
         return get_plugin_top(p->next, name);
}

void *set_plugin_top(plugins *p, char *name, int top)
{
   ...
   if (!strcmp(p->next->plugin_name, name))
         ...
         return set_plugin_top(p->next, name, top);
}

int get_plugin_right(plugins *p, char *name)
{
   if (!p->next) return 0;
   ...
   if (!strcmp(p->next->plugin_name, name))
      ...
      return get_plugin_right(p->next, name);
}


void *set_plugin_right(plugins *p, char *name, int right)
{
   if (!p->next) return 0;
   ...
   if (!strcmp(p->next->plugin_name, name))
         ...
         return set_plugin_right(p->next, name, right);
}
int get_plugin_bottom(plugins *p, char *name)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
          return p->next->bottom;
   else  
         return get_plugin_bottom(p->next, name);
}


void *set_plugin_bottom(plugins *p, char *name, int bottom)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
         p->next->bottom = bottom;
   else  
         return set_plugin_bottom(p->next, name, bottom);
}
int get_plugin_left(plugins *p, char *name)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
         ...
         return get_plugin_left(p->next, name);
}


void *set_plugin_left(plugins *p, char *name, int left)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
         ...
         return set_plugin_left(p->next, name, left);
}

BOOL *is_plugin_loaded(plugins *p, char *name)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
         return p->next->loadedonce;
   else  
         return is_plugin_loaded(p->next, name);
}
BOOL *plugin_loaded(plugins *p, char *name)
{
   if (!p->next) return FALSE;
   ...
   if (!strcmp(p->next->plugin_name, name))
         return p->next->loadedonce = TRUE;
   else  
         return plugin_loaded(p->next, name);
}
void insert_plugin(plugins *p, char *file_name,
		   char *plugin_name, void *handle, int type,int num, char *file_path)
{
    if (p->next)
        insert_plugin(p->next, file_name, plugin_name, handle, type, 
                               (p->type == type) ? num+1 : num,file_path);
    else
    {
        p->next = malloc(sizeof(plugins));
        p->next->type = type;
        p->next->handle = handle;
        p->next->loadedonce = FALSE;
        p->next->file_path = malloc(strlen(file_path)+1);
        strcpy(p->next->file_path, file_path);
...
void *reload_plugin(plugins *p, char *name)
{
   if (!p->next) return NULL;
   void (*CloseDll)(void);
   while(p->next->plugin_name[strlen(p->next->plugin_name)-1] == ' ') 
   ...
   if (!strcmp(p->next->plugin_name, name))
   {
         ...
         CloseDll = (void(__cdecl*)())GetProcAddress( p->next->handle, "CloseDLL");
         CloseDll();
         while (FreeLibrary(p->next->handle) != 0)
         {
             CloseDll = (void(__cdecl*)())GetProcAddress( p->next->handle, "CloseDLL");
             if (CloseDll) { CloseDll(); }
         }
         p->next->handle = NULL;
         p->next->handle = LoadLibrary(p->next->file_path);
         return p->next->handle;
   }
   else  
         return reload_plugin(p->next, name);
}

void *plugin_filename_by_name(plugins *p, char *name)
{
   if (!p->next) return NULL;
...
void exec_config(char *name)
{
   ...
   tempdir = plugin_filename_by_name(liste_plugins, name);
   extern BOOL KailleraConfigOpen;
   KailleraConfigOpen = TRUE;
   int i ;
      ...
      switch (PluginInfo.Type)
                {
                    ...
                    break;
                    case PLUGIN_TYPE_CHEAT:
                       if (!emu_launched) {
                          initiateCheat = (BOOL (__cdecl *)(CHEAT_INFO Cheat_Info))GetProcAddress( handle, "InitiateCheat" );
                          if (!initiateCheat(dummy_cheat_info)) {
			                    ShowMessage("Failed to initialize cheat plugin.");
		                  }
		               }
                       ...
                       if (dllConfig) dllConfig(hwnd_plug); 
                       if (!emu_launched) {
                          closeDLL_cheat = (void(__cdecl*)())GetProcAddress( handle, "CloseDLL");
		                  if (closeDLL_cheat) closeDLL_cheat();
		               }
                    break;
                    case PLUGIN_TYPE_GFX:  
//                      reload_plugin(liste_plugins,handle);
                     if (!emu_launched && !is_plugin_loaded(liste_plugins,name)) {
                                               
                                initiateGFX = (BOOL(__cdecl*)(GFX_INFO Gfx_Info))GetProcAddress(handle, "InitiateGFX");
                                 romClosed_gfx = (void(__cdecl*)())GetProcAddress(handle, "RomClosed");
                                ...
                                } else { plugin_loaded(liste_plugins,name); }
                        ...
                        if (!emu_launched) {
                             ...
                             MoveWindow(mainHWND, RealRect.left, RealRect.top, RealRect.right-RealRect.left, RealRect.bottom-RealRect.top, TRUE);
...
int check_plugins()
{
   void *handle_gfx, *handle_input, *handle_sound,*handle_rsp,*handle_cheat ;
   ...
   handle_cheat = get_handle(liste_plugins, cheat_name);
   if (!handle_cheat) {
         ShowMessage("Choose Cheat plugin in Configuration Dialog.");
		 return (0);
   }
   return 1;
}
...
//                            if(!is_plugin_loaded(liste_plugins, gfx_name))
//                            {
...
/*                                else {
                                      ...
                                      GetClientRect(mainHWND, &dummyRect);
                                      set_plugin_top(liste_plugins,gfx_name,dummyRect.top);
                                      set_plugin_left(liste_plugins,gfx_name,dummyRect.left);
                                      set_plugin_right(liste_plugins,gfx_name,dummyRect.right);
                                      set_plugin_bottom(liste_plugins,gfx_name,dummyRect.bottom);
                                      AdjustWindowRectEx(&dummyRect,GetWindowLong(mainHWND,GWL_STYLE),TRUE,GetWindowLong(mainHWND,GWL_STYLE));
                                      SetWindowPos(mainHWND,0,RealRect.left,RealRect.top, dummyRect.right-dummyRect.left, dummyRect.bottom-dummyRect.top, SWP_NOMOVE | SWP_FRAMECHANGED);  
                                      
                                      plugin_loaded(liste_plugins, gfx_name);
                                      }      
                             }
                             else
                             {
                                   GetClientRect(mainHWND, &dummyRect);
                                   dummyRect.top = get_plugin_top(liste_plugins,gfx_name);
                                   dummyRect.left = get_plugin_left(liste_plugins,gfx_name);
                                   dummyRect.right = get_plugin_right(liste_plugins,gfx_name);
                                   dummyRect.bottom = get_plugin_bottom(liste_plugins,gfx_name); 
...
int load_cheat(void *handle_cheat )
{
    if (handle_cheat)
     {
    closeDLL_cheat = (void (__cdecl *)())GetProcAddress(handle_cheat, "CloseDLL" );
	initiateCheat = (BOOL (__cdecl *)(CHEAT_INFO))GetProcAddress(handle_cheat, "InitiateCheat" );
	romClosed_cheat = (void (__cdecl *)())GetProcAddress(handle_cheat, "RomClosed" );
	romOpen_cheat = (void (__cdecl *)())GetProcAddress(handle_cheat, "RomOpen" );
	applyCheats = (void (__cdecl *)())GetProcAddress(handle_cheat, "ApplyCheatsOnVI" );


	if (closeDLL_cheat == NULL) closeDLL_cheat = dummy_void;

	if (romClosed_cheat == NULL) romClosed_cheat = dummy_void;
	if (romOpen_cheat == NULL) romOpen_cheat = dummy_void;
	if (applyCheats == NULL) applyCheats = dummy_void;
	
    cheat_info.hwnd = mainHWND;
    cheat_info.hinst = app_hInstance;
//    cheat_info.MemoryBswaped = TRUE;
    cheat_info.HEADER = rom;
        
    cheat_info.RDRAM = (BYTE*)rdram;
  
  
    initiateCheat(cheat_info);
   
    }
    else
     {

	closeDLL_cheat = dummy_void;
	initiateCheat = dummy_initiateCheat;

	romClosed_cheat = dummy_void;
	romOpen_cheat = dummy_void;
     }
     return 0;
}
...
int load_plugins()
{
   ...
   if (!Config.OverwritePluginSettings) 
   {
       ...
       handle_gfx = reload_plugin(liste_plugins, TempRomSettings.GfxPluginName);
       if (handle_gfx==NULL) {handle_gfx = reload_plugin(liste_plugins, gfx_name);}
       else {sprintf(gfx_name,TempRomSettings.GfxPluginName);}
   
       handle_input = reload_plugin(liste_plugins, TempRomSettings.InputPluginName); 
       if (handle_input==NULL) handle_input = reload_plugin(liste_plugins, input_name);
       else {sprintf(input_name,TempRomSettings.InputPluginName);}
   
       handle_sound = reload_plugin(liste_plugins, TempRomSettings.SoundPluginName); 
       if (handle_sound==NULL) handle_sound = reload_plugin(liste_plugins, sound_name);
       else {sprintf(sound_name,TempRomSettings.SoundPluginName);}
   
       handle_rsp = reload_plugin(liste_plugins, TempRomSettings.RspPluginName); 
       if (handle_rsp==NULL) handle_rsp = reload_plugin(liste_plugins, rsp_name);
       else {sprintf(rsp_name,TempRomSettings.RspPluginName);}
       
       handle_cheat = reload_plugin(liste_plugins, TempRomSettings.CheatPluginName); 
       if (handle_cheat==NULL) handle_cheat = reload_plugin(liste_plugins, cheat_name);
       else {sprintf(cheat_name,TempRomSettings.CheatPluginName);}
   ...
   else 
   {
       ...
       handle_gfx = reload_plugin(liste_plugins, gfx_name);
       handle_input = reload_plugin(liste_plugins, input_name);
       handle_sound = reload_plugin(liste_plugins, sound_name);
       handle_rsp = reload_plugin(liste_plugins, rsp_name);
       handle_cheat = reload_plugin(liste_plugins, cheat_name);
   }
  // SetupDummyInfo();
   if(check_plugins() == 1)
   {
   ...
   load_rsp(handle_rsp);
   ShowInfo("Loading cheat - %s",cheat_name);
   load_cheat(handle_cheat);       
   return (1);
   }
}
...
void resumeEmu()
{
    /* extern KailleraPacket NetPlayInformation[8]; 
    extern BOOL KailleraGamePaused;
    int MessagePosition; */
    if (KailleraRunningGame/* && KailleraGamePaused*/)
    {
       /* NetPlayInformation[0].KailleraMessageType = 0x04;
...
void pauseEmu()
{
   /* extern KailleraPacket NetPlayInformation[8]; 
    ...
    {
        /*
        NetPlayInformation[0].KailleraMessageType = 0x03;
        NetPlayInformation[0].KailleraMessage = 0x00;
        KailleraModifyPlayValues(NetPlayInformation,5);
        for (MessagePosition = 0; MessagePosition < 30;  MessagePosition++)
        {
...
BOOL StartRom(char *fullRomPath)
{
     ...
     if (emu_launched) {/*closeRom();*/ ShowInfo("Close ROM?"); CreateThread(NULL, 0, closeRom, NULL, 0, &Id);}
     if (!emu_launched) {
                         //Makes window not resizable                         
                         // SetActiveWindow(mainHWND);
                          if (appMaximized || appMinimized)
                          {
//                             ShowInfo("Restore Window");
                             tempMaximized = appMaximized;
                             SendMessage(mainHWND, WM_SYSCOMMAND, SC_RESTORE, 0);
                          }
                          else { GetWindowRect(mainHWND,&RealRect); }
                          SetWindowPos(mainHWND,0,((tempMaximized) ? 0 : RealRect.left),((tempMaximized) ? 0 : RealRect.top),0,0,SWP_NOSIZE);
                          SetWindowLong(mainHWND, GWL_STYLE, GetWindowLong( mainHWND, GWL_STYLE ) & ~WS_THICKFRAME & ~WS_MINIMIZE & ~WS_MAXIMIZE & ~WS_MAXIMIZEBOX );
                          SetActiveWindow(mainHWND);
...
//   int browserwidths[] = {400, -1};              //Rombrowser statusbar
   ...
   if (tempMaximized)
   {
      ...
      SendMessage(mainHWND, WM_SYSCOMMAND, SC_MAXIMIZE, 0);
   }
   SetWindowPos(mainHWND, HWND_NOTOPMOST,  (RealRect.left < 0 ?  0 : RealRect.left), (RealRect.top < 0 ?  0 : RealRect.top), RealRect.right-RealRect.left, RealRect.bottom-RealRect.top,  SWP_FRAMECHANGED | (tempMaximized == TRUE ? ( SWP_NOSIZE | SWP_NOMOVE) : 0));  //Set on top
   return 0;
}
...
void setDefaultPlugins() 
{
      ...
      ReadCfgString("Plugins","Cheat","",cheat_name);
      rewind_plugin();
}           
...
void EnableEmulationMenuItems(BOOL flag)
{
   ...
   if (flag) {
      ...
      EnableMenuItem(hSubMenu,9,MF_BYPOSITION | MF_ENABLED);    //Record Menu
...
static DWORD WINAPI ThreadFunc(LPVOID lpParam)
{
    ...
    romOpen_input();
    ShowInfo("Rom open cheat...");
    romOpen_cheat();
    ShowInfo("Rom open gfx....");
    romOpen_gfx();

    extern BOOL KailleraRunningGame;
    extern int KailleraGamePlayerCount;
    int i;
    //GetWindowRect(mainHWND, &RequestRect); 
    if (KailleraRunningGame)
    {
  /*    if (Controls[0].Present == FALSE)
      {
         kailleraChatSend("Controller is not enabled, Desynch Detected!");
         SendMessage(mainHWND, WM_CLOSE, 0, 0);
         ShowMessage("Please enable 'controller 1' inside of your input plugin configuration.");
      } */
      for (i=0; i < 4; i++)
      {
      ...
      for (i=0; i < KailleraGamePlayerCount; i++)
      {  
    ...
    ShowInfo("Emu thread: romClosed (RSP plugin)");
    romClosed_cheat();
    ShowInfo("Emu thread: romClosed (cheat plugin)");
    romClosed_gfx();
    ShowInfo("Emu thread: romClosed (gfx plugin)");
    
   // closeDLL_cheat();
    ShowInfo("Emu thread: cheat plugin closed");
   // closeDLL_RSP();
    ShowInfo("Emu thread: RSP plugin closed");
   // closeDLL_input();
    ShowInfo("Emu thread: input plugin closed");
   // closeDLL_audio();
    ...
    // This whole thing is the incorrect way to load plugins, but for now...
    // I am going to rewrite the plugin system one at a time. The graphics
    // plugin is acutally loaded correctly.
     ...
     switch(Message)
     {
       ...
       case WM_KEYDOWN:
        ...
        case WM_KEYUP:
                ...
                break;   
        case WM_CHAR:
               if (KailleraChatEnabled)
               {
                    KailleraChatKeyDown(wParam,lParam);
               }
               break;
        case WM_NOTIFY:
        ...
        case WM_SIZE:
             ...
             isMaximized = GetWindowLong( mainHWND, GWL_STYLE );
             ...
             appMinimized = ((isMaximized & WS_MINIMIZE) ? TRUE : FALSE);
             appMaximized = ((isMaximized & WS_MAXIMIZE) ? TRUE : FALSE);
            break;  
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                ...
                case IDGAMESHARK:
                     ...
                     MAKEINTRESOURCE(IDD_GAMESHARK), hwnd, 0);
                     break;
                case IDRSPCONFIG:
                     hwnd_plug = hwnd;
                ...
                case ID_LOG_WINDOW:
                     ShowHideLogWindow();
                     break; 
                case ID_KAILLERACLIENT:
                     TargetKailleraClient = Config.KailleraClient;
                     KailleraThreadHandle = CreateThread(NULL, 0, LaunchKaillera, NULL, 0, &KailleraThread);
                     break;
                case ID_KAILLERACHAT:
                    if (KailleraRunningGame)
                    {
                      SetStatusTranslatedString( hStatus, 0, "Enter a message then press enter to send it. (ESC to cancel)");
                      KailleraChatEnabled = TRUE;
                    }
                    break;
                case ID_SUPRACLIENT:
                     TargetKailleraClient = Config.SupraClient;                     
                     KailleraThreadHandle = CreateThread(NULL, 0, LaunchKaillera, NULL, 0, &KailleraThread);
                     break;
...
if (GuiDisabled()) {
	...
	wc.hIconSm		 = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_M64ICONSMALL));
	wc.hCursor		 = LoadCursor(NULL, IDC_ARROW);
	wc.hbrBackground = 0; //(HBRUSH)(COLOR_WINDOW+11);
    ...
    hwnd = CreateWindowEx(
		...
		g_szClassName,
		MUPEN_VERSION,
		WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_EX_TOPMOST | (Config.isMaximized == 1 ? WS_MAXIMIZE : 0),
		(Config.WindowPosX < 0 ?  0 : Config.WindowPosX), (Config.WindowPosY < 0 ?  0 : Config.WindowPosY), Config.WindowWidth, Config.WindowHeight,
		NULL, NULL, hInstance, NULL);
    ...
    appMaximized = (Config.isMaximized == 1 ? TRUE : FALSE);
    mainHWND = hwnd ;
...
// Why does this code even make sense? Delete the menu item if it exists? How about disabling it!
    
/*
	if (!isKailleraExist())	{
                DeleteMenu( GetMenu(hwnd), ID_KAILLERACLIENT, MF_BYCOMMAND);
    }   
*/    
    SetupDummyInfo(); 
    ...
    ShowInfo("This is the debug window, this window is not the emulator.");
    SetStatusTranslatedString( hStatus, 0, MUPEN_VERSION );
